...
/**
 * Function load more for search page
 *
 * @return string Return the posts html.
 */
function Btemptd_Home_Load_more()
{
    if (isset($_POST['nonce']) && !empty($_POST['nonce'])) {
        $nonce = $_POST['nonce'];
    }
    ...
    $home_recent_posts = Btemptd_Query_posts(
        array(
            'post_type'        => array('post'),
            'posts_per_page'   => 3,
    ...
    if (!empty($home_recent_posts)) {
        ob_start();
        ?>
        <div class="explore-blog explore-see-more">
        <div class="explore-blog--bg">
        <div class="explore-blog--wrapper">
        <?php foreach($home_recent_posts as $key =>$recent_post):
            $thumbnail_id  = get_post_thumbnail_id($recent_post->ID);
            $thumbnail_url = Btemptd_Get_image(wp_get_attachment_image_src($thumbnail_id, 'full'));
            $thumbnail_alt = Btemptd_Get_Image_alt($thumbnail_id, 'featured-img');
            $categories    = Btemptd_Get_Primary_category($recent_post->ID);
...
add_action('wp_ajax_nopriv_btemptd_home_load_more', 'Btemptd_home_load_more');
add_action('wp_ajax_btemptd_home_load_more', 'Btemptd_home_load_more');
                            ...
                            <div class="explore-blog--box box-shadow-right">
                                ...
                                <div class="explore-blog--content blog-pagination">
                                    <div class="blog-pagination-content">
                                        ...
                                        <div class="explore-blog--content__title">
                                            <a href="<?php echo esc_url(get_permalink($cat_post->ID));?>">
                                                <?php echo esc_attr(Btemptd_Limit_text(get_the_title($cat_post->ID), 70));?>
                                            </a>
                                        </div>
                                    </div>
 ...
 * @return string Return the posts html.
 */
function Btemptd_Search_Load_more()
{
...
add_action('wp_ajax_nopriv_btemptd_search_load_more', 'Btemptd_Search_Load_more');
add_action('wp_ajax_btemptd_search_load_more', 'Btemptd_Search_Load_more');
