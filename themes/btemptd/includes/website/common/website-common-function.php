...
if (!function_exists('btemptd_posted_on')) {

    function btemptd_posted_on()
    {
        ...
        $posted_on = sprintf(
            esc_html_x('Posted on %s', 'post date', 'btemptd'),
 ...
 * @return html
 */
if (!function_exists('btemptd_posted_by')) {

    function btemptd_posted_by()
    {
        $byline = sprintf(
            esc_html_x('by %s', 'post author', 'btemptd'),
 ...
 * @return void
 */
if (!function_exists('btemptd_entry_footer')) {

    function btemptd_entry_footer()
    {
        // Hide category and tag text for pages.
        if ('post' === get_post_type()) {

            $categories_list = get_the_category_list(esc_html__(', ', 'btemptd'));
            if ($categories_list) {
                printf('<span class="cat-links">' . esc_html__('Posted in %1$s', 'btemptd') . '</span>', $categories_list); // phpcs:ignore
            }

            $tags_list = get_the_tag_list('', esc_html_x(', ', 'list item separator', 'btemptd'));
            if ($tags_list) {
                printf('<span class="tags-links">' . esc_html__('Tagged %1$s', 'btemptd') . '</span>', $tags_list); // phpcs:ignore
            }
        }
        ...
        if (!is_single() && !post_password_required() && (comments_open() || get_comments_number())) {
            ...
            comments_popup_link(
                sprintf(
                    wp_kses(
                        __('Leave a Comment<span class="screen-reader-text"> on %s</span>', 'btemptd'),
                        array(
                            'span' => array(
                                'class' => array(),
                            ),
        ...
        edit_post_link(
            sprintf(
                wp_kses(
                    __('Edit <span class="screen-reader-text">%s</span>', 'btemptd'),
                    array(
                        'span' => array(
                            'class' => array(),
                        ),
 ...
 * @return html
 */
if (!function_exists('btemptd_post_thumbnail')) {

    function btemptd_post_thumbnail()
    {
...
function btemptd_widgets_init()
{
    register_sidebar(
        array(
        'name' => esc_html__('Sidebar', 'btemptd'),
        'id' => 'sidebar-1',
...
function btemptd_paging_nav()
{
  ...
  */
function btemptd_exclude_posts_from_specific_category( $query )
{
...
add_action('pre_get_posts', 'btemptd_exclude_posts_from_specific_category');
 ...
 * @return void
 */
function btemptd_add_taxonomies_to_pages()
{
    register_taxonomy_for_object_type('category', 'page');
}
add_action('init', 'btemptd_add_taxonomies_to_pages');
