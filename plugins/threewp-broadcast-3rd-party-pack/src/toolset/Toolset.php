	...
	extends \threewp_broadcast\premium_pack\base
{
	...
	**/
	public function maybe_save_relationships( $action )
	{
		...
		// Find all parent associations with this group ID.
		...
		foreach( $associations as $association_id => $association )
		{
			...
			foreach( [ 'child_id', 'parent_id', 'intermediary_id' ] as $type )
			{
				$id = $association->$type;
				if ( $id < 1 )
					continue;
	...
	**/
	public function maybe_restore_relationships( $action )
	{
		...
		if ( $group )
			$this->delete_parent_association( $group->group_id );
		...
		foreach( $parent_groups as $parent_group )
		{
			...
			$child_groups->set( $parent_group_id, $child_group );
		}

		$this->debug( 'New child groups: %s', $child_groups );
		...
		foreach( $associations as $association )
		{
			...
			foreach( [ 'child_id', 'parent_id', 'intermediary_id' ] as $type )
			{
				...
				if ( $old_group_id < 1 )
					continue;
				$child_group = $child_groups->get( $old_group_id );
				if ( ! $child_group )
					continue;
				$found = true;
	...
	**/
	...
	/**
		@brief		Delete associations where this group_id is the parent.
		@since		2021-04-20 21:21:09
	**/
	public function delete_parent_association( $group_id )
	{
		global $wpdb;
		$table = $wpdb->prefix . 'toolset_associations';
		$query = sprintf( "DELETE FROM `%s` WHERE `parent_id` = '%s'", $table, $group_id );
		$this->debug( $query );
		$this->query( $query );
	}
	...
	**/
	public function get_relationship_associations( $group_id )
	{
		global $wpdb;
		$table = $wpdb->prefix . 'toolset_associations';
		$query = sprintf( "SELECT * FROM `%s` WHERE `parent_id` = '%s' OR `intermediary_id` = '%s' ", $table, $group_id, $group_id );
		$this->debug( $query );
		$results = $wpdb->get_results( $query );
		return $results;
	}
